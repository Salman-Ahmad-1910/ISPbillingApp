...
func applyPendingFilters(db *gorm.DB, c *gin.Context, companyUUID uuid.UUID) *gorm.DB {
	// Pending = any subscriber with a stored remaining balance. The balance is
	// established by the Bill Creator (full package fee), the subscriber's
	// "Create balance" option (prorated fee) and reduced by received payments.
	db = db.Where("company_id = ? AND deleted_at IS NULL", companyUUID)
	db = db.Where("remaining_amount > 0")
...
// GetPendingSubscribers returns the paginated list of subscribers who still
// have a remaining balance (pending) for the company, along with the total
// pending count and total pending amount.
