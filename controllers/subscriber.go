...
func DeleteSubscriber(c *gin.Context) {
	...
	if err := config.DB.Where("id = ? AND company_id = ?", id, companyID).Delete(&models.Subscriber{}).Error; err != nil {
		utils.ErrorResponse(c, 500, "Failed to delete subscriber", err.Error())
		return
	}
