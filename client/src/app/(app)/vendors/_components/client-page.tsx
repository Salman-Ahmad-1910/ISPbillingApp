...
export function ClientPage({ data }: ClientPageProps) {
  ...
  const vendorMutation = useMutation({
    mutationFn: async (values: VendorFormValues) => {
      if (selectedVendor) {
        // Update vendor
        const response = await api.put(`/crm/vendors/${selectedVendor.id}`, values);
        return response.data;
      } else {
        // Create vendor
        const response = await api.post('/crm/vendors', values);
        return response.data;
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['crm/vendors', companyId] });
      setIsFormOpen(false);
  ...
  const deleteVendorMutation = useMutation({
    mutationFn: async (vendorId: string) => {
      await api.delete(`/crm/vendors/${vendorId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['crm/vendors', companyId] });
