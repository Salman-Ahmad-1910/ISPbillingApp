...
export default function DealersDefaultersPage() {
  ...
  const overdueRecords: DefaulterRecord[] = data
    .filter((item) => {
    ...
    .map((item) => {
      ...
      return {
        ...
        daysOverdue,
        category: daysOverdue >= 90 ? 'Bad Debt' as const : 'Defaulter' as const,
      };
    });

  const filteredData = overdueRecords.filter((item) => {
    ...
    const afterFrom = due >= from;
    const beforeTo = due <= to;
    const catMatch = reportType === 'all' || item.category === reportType;

    return afterFrom && beforeTo && catMatch;
  });
  ...
  const exportExcel = () => {
    ...
    link.href = url;
    link.setAttribute('download', `dealers-defaulters-${format(new Date(), 'yyyy-MM-dd')}.csv`);
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex items-center gap-3 no-print">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Dealers Defaulters</h1>
          <p className="text-sm text-muted-foreground">View defaulting dealers and bad debts</p>
        </div>
      </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={reportType} onValueChange={setReportType}>
                ...
                <SelectContent portal={false}>
                  <SelectItem value="defaulters">Defaulters</SelectItem>
                  <SelectItem value="Bad Debt">Bad debts</SelectItem>
                </SelectContent>
              </Select>
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 no-print">
        ...
        <div className="group rounded-xl border bg-card p-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-muted-foreground">Receivable Amount</p>
              <p className="text-2xl font-bold mt-1">Pkr {totalReceivable.toLocaleString('en-IN')}</p>
            </div>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            {loading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                No defaulter records found for the selected criteria.
              </div>
            ) : (
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {filteredData.map((item, i) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <span className={item.daysOverdue >= 90 ? 'text-destructive font-bold' : 'text-amber-600 font-medium'}>
                          {item.daysOverdue}
                        </span>
                      </TableCell>
                      <TableCell>
                        <Badge variant={item.category === 'Bad Debt' ? 'destructive' : 'default'}>
                          {item.category}
                        </Badge>
                      </TableCell>
                    </TableRow>
