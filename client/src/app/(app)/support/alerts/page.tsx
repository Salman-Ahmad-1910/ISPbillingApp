...
import { BellRing, Plus, Loader2, Bell, Smartphone, MessageCircle } from 'lucide-react';
...
export default function AlertsPage() {
  ...
  const kpiData = {
    total: alerts.length,
    smsEnabled: alerts.filter(a => a.smsEnabled).length,
    whatsAppEnabled: alerts.filter(a => a.whatsAppEnabled).length,
  };
  ...
  const initializeTemplates = async () => {
    ...
    try {
      ...
      const createdAlerts = responses.map(response => response.data);
    ...
    return (
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Custom Alerts & Notifications</h1>
            <p className="text-sm text-muted-foreground">Set up automated WhatsApp and SMS alerts.</p>
          </div>
        </div>
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
            ...
            <DialogContent className="rounded-xl shadow-lg">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-4">
                  ...
                  <Button onClick={handleCreateAlert} disabled={isSaving} className="bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-sm hover:shadow-md transition-all duration-300">
          ...
          {alerts.length === 0 && (
            <Button variant="outline" onClick={initializeTemplates} disabled={isSaving} className="transition-all duration-300 hover:shadow-md">
              Initialize Defaults
            </Button>
          )}
          <Button onClick={handleSaveChanges} disabled={isSaving || alerts.length === 0} className="bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-sm hover:shadow-md transition-all duration-300">
            {isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="group rounded-xl border bg-card p-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">{kpiData.total}</p>
            </div>
        ...
        <div className="group rounded-xl border bg-card p-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-muted-foreground">SMS Enabled</p>
              <p className="text-2xl font-bold mt-1">{kpiData.smsEnabled}</p>
            </div>
        ...
        <div className="group rounded-xl border bg-card p-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-muted-foreground">WhatsApp Enabled</p>
              <p className="text-2xl font-bold mt-1">{kpiData.whatsAppEnabled}</p>
            </div>
      ...
      {alerts.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {alerts.map((alert) => (
            <Card key={alert.id || alert.templateId || Math.random().toString()} className="transition-all duration-300 hover:shadow-md">
              <CardHeader>
