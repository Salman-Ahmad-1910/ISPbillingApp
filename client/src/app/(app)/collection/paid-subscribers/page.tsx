...
import { CheckCircle2, ArrowLeft, Users, Wallet, Loader2, Search } from 'lucide-react';
...
export default function PaidSubscribersPage() {
  ...
  // Paid = dues cleared (remaining_amount <= 0). Advance (overpaid) subscribers
  // are included here as well as on the Advance page.
  const paidSubscribers = useMemo(
    () => (connections as Connection[]).filter(c => (Number(c.remainingAmount) || 0) <= 0),
    [connections],
  );

  const filteredSubscribers = useMemo(
    () => applyConnectionFilters(paidSubscribers, filters, search),
    [paidSubscribers, filters, search],
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Paid Subscribers</h1>
          <p className="text-sm text-muted-foreground">Subscribers who have cleared their dues</p>
        </div>
      </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold mt-1">{paidSubscribers.length}</p>
              </div>
              <div className="rounded-lg bg-gradient-to-br from-sky-500 to-blue-600 p-2.5 text-white shadow-sm">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold mt-1">
                  PKR {paidSubscribers.reduce((sum, c) => sum + getPackagePrice(c), 0).toLocaleString()}
                </p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold mt-1">
                  PKR {paidSubscribers.reduce((sum, c) => sum + Math.abs(Number(c.remainingAmount) || 0), 0).toLocaleString()}
                </p>
              </div>
              <div className="rounded-lg bg-gradient-to-br from-sky-500 to-blue-600 p-2.5 text-white shadow-sm">
                <CheckCircle2 className="h-5 w-5" />
        ...
        <CardContent className="pt-6">
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              <CheckCircle2 className="h-10 w-10 opacity-30 mx-auto mb-3" />
              <p className="text-sm font-medium">No paid subscribers</p>
              <p className="text-xs mt-1">No subscribers match the current filters.</p>
            </div>
          ) : (
                ...
                <TableBody>
                   {pagedSubscribers.map((c, i) => (
                     <TableRow key={c.id}>
                      ...
                      <TableCell>
                        {c.paymentStatus === 'advance' ? (
                          <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-300">Advance</Badge>
                        ) : (
                          <Badge variant="outline" className="bg-sky-50 text-sky-700 border-sky-300">Paid</Badge>
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
