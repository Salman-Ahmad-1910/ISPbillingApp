...
import { Wifi, ArrowLeft, Users, Loader2, Search } from 'lucide-react';
...
export default function ActiveSubscribersPage() {
  ...
  const activeSubscribers = useMemo(
    () => (connections as Connection[]).filter(c => c.status === 'active'),
    [connections],
  );

  const filteredSubscribers = useMemo(
    () => applyConnectionFilters(activeSubscribers, filters, search),
    [activeSubscribers, filters, search],
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Active Subscribers</h1>
          <p className="text-sm text-muted-foreground">Subscribers with an active connection</p>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold mt-1">
                  PKR {activeSubscribers.reduce((sum, c) => sum + getPackagePrice(c), 0).toLocaleString()}
                </p>
              </div>
        ...
        <CardContent className="pt-6">
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              <Wifi className="h-10 w-10 opacity-30 mx-auto mb-3" />
              <p className="text-sm font-medium">No active subscribers</p>
                ...
                <TableBody>
                   {pagedSubscribers.map((c, i) => (
                     <TableRow key={c.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-300">Active</Badge>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
