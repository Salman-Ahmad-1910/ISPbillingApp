...
import { Loader2, Package, CheckCircle, DollarSign } from 'lucide-react';
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Define pricing structures and plans for your products.</p>
        </div>
      </div>
        ...
        <div className="group rounded-xl border bg-card p-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{Array.isArray(plans) ? plans.length : 0}</p>
            </div>
          </div>
        </div>
        <div className="group rounded-xl border bg-card p-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                {Array.isArray(plans) ? plans.filter((p: any) => p.status === 'active' || p.status === 'Active').length : 0}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                PKR {Array.isArray(plans) && plans.length > 0
                  ? Math.round(plans.reduce((s: number, p: any) => s + (Number(p.price) || 0), 0) / plans.length).toLocaleString()
                  : 0}
              </p>
            </div>
          </div>
