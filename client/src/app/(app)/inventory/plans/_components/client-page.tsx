...
export function ClientPage({ data }: ClientPageProps) {
  ...
  const filteredData = useMemo(() => plans.filter(
    (plan) =>
  ...
  const handleSave = async (formData: PlanFormValues) => {
    ...
    try {
      ...
      if (selectedPlan) {
        await api.put(`/inventory/plans/${selectedPlan.id}?companyId=${companyId}`, planData);
        toast({ title: 'Success', description: 'Plan updated successfully.' });
      } else {
        await api.post(`/inventory/plans?companyId=${companyId}`, { ...planData, companyId: companyId! });
        toast({ title: 'Success', description: 'Plan added successfully.' });
      }
      queryClient.invalidateQueries({ queryKey: ['inventory/plans', companyId] });
      setIsFormOpen(false);
  ...
  const handleDelete = async () => {
    if (selectedPlan) {
      try {
        await api.delete(`/inventory/plans/${selectedPlan.id}?companyId=${companyId}`);
            ...
            <DialogContent className="rounded-xl shadow-lg">
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  <div className="rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 p-1.5 text-white shadow-sm">
                  ...
                  <span>{selectedPlan ? 'Edit' : 'Add'} Plan</span>
                </DialogTitle>
              </DialogHeader>
              <PlanForm
                plan={selectedPlan}
        ...
        <DataTable columns={columns} data={getPaginatedData()} />
                ...
                <div className="flex items-center justify-between mt-4">
                    <div className="text-sm text-muted-foreground">
                        Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, filteredData.length)} of {filteredData.length} plans
                    </div>
