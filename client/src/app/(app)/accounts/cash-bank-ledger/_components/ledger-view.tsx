...
export function LedgerView({ initialData, accountType }: LedgerViewProps) {
  ...
  const tableData = useMemo(() => getPaginatedData(), [getPaginatedData]);
  ...
  const handleSave = useCallback(async (formData: LedgerEntryFormValues) => {
    setIsSaving(true);
    try {
      if (selectedEntry) {
  ...
  }, [selectedEntry, companyId, accountType, queryClient, toast]);
  ...
  const openDeleteDialog = useCallback((entry: LedgerEntry) => {
    setSelectedEntry(entry);
    setIsDeleteDialogOpen(true);
  }, []);


  const columns = useMemo(() => getColumns({ onEdit: handleEdit, onDelete: openDeleteDialog }), [handleEdit, openDeleteDialog]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4 pt-4 px-6 w-full">
        ...
        <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
          <DialogTrigger asChild>
            <Button className="mr-6 mt-4 bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-sm hover:from-emerald-600 hover:to-green-700">
      ...
      <DeleteAlertDialog
        ...
        itemName={selectedEntry?.description}
      />
    </div>
  );
}
